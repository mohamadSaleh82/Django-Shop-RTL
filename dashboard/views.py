...
def edit_product(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'محصول با موفقیت به‌روزرسانی شد', 'success')
            return redirect('dashboard:products')
    else:
...
def add_category(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'دسته با موفقیت افزوده شد!')
            return redirect('dashboard:add_category')
    else:
        form = AddCategoryForm()
    context = {'title': 'اضافه کردن دسته', 'form': form}
...
def orders(request):
    ...
    context = {'title': 'سفارش ها', 'orders': orders}
...
def order_detail(request, id):
    ...
    context = {'title': 'مشخصات سفارش', 'items': items, 'order': order}
